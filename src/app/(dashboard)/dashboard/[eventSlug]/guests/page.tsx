...
export default function GuestsPage() {
  ...
  const enrichedGuests = useMemo(() => {
    ...
    return guests.map((g) => {
      ...
      if (g.isPlusOne && g.plusOneOfGuestId) {
        plusOneLabel = `↳ +1 of ${guestNameById[g.plusOneOfGuestId] ?? "—"}`;
      ...
      return {
        ...g,
        invitationTitle: g.invitationId
          ? invitationMap[g.invitationId]
          : undefined,
        menuOptionName: g.menuOptionId
          ? menuOptionMap[g.menuOptionId]
          : undefined,
        drinkOptionName: g.drinkOptionId
          ? drinkOptionMap[g.drinkOptionId]
          : undefined,
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          guests === undefined
            ...
            : guests.length === 0
              ? "Everyone invited to this event."
              : `${guests.length} ${guests.length === 1 ? "guest" : "guests"} · ${attendingCount} attending`
        }
        actions={
      ...
      {/* Thrown query errors surface through the (dashboard) route-group
          error boundary, which renders a StateBlock kind="error" with retry. */}
      {guests === undefined ? (
        <div
          className="space-y-3"
          role="status"
        ...
        <Panel padded={false}>
          <StateBlock
            ...
            icon={Users}
            title="No guests yet"
            description="Add guests here, then group them into invitations to share."
            action={{
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Guests can be grouped into an invitation later.
            </DialogDescription>
          </DialogHeader>
